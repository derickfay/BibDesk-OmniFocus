using terms from application "BibDesk"
	on perform BibDesk action with publications thePubs for script hook theScriptHook
		...
		-- 		display dialog (path of theDocument) as string
		if theName is not "Close Editor Window" then return
		
		repeat with thePub in thePubs
			set currentKeywords to get keywords of thePub as string
			if currentKeywords contains "oft" then
...
" & theLink
				...
				repeat with theFile in linkedFiles
					set linkText to linkText & "
file://" & encode_text(theFile, false, true)
				end repeat
...
" & (au & " (" & ye & "). " & ti & "."), note:"x-bdsk://" & ci & linkText}
				...
				(*				tell front document of application "OmniFocus"
					parse tasks into it with transport text (au & " " & ye & " " & ti & " //" & "x-bdsk://" & ci)
				end tell
				*)
				set keywords of thePub to replace_chars(currentKeywords, "oft", "")
